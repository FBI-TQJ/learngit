...
namespace TQJProject.Tests
{
    public abstract class TQJProjectTestBase : AbpIntegratedTestBase<TQJProjectTestModule>
    {
        ...
        protected void UsingDbContext(Action<TQJProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TQJProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TQJProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TQJProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TQJProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TQJProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TQJProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TQJProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TQJProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TQJProjectDbContext, Task<T>> func)
        {
            T result;
